...
O objetivo do trabalho é uma análise da integridade de elementos de barreira do CSB (Conjunto Solidário de Barreiras) secundário durante a fase de perfuração. Durante o ciclo de vida de um poço, o mesmo está sujeito a diversos carregamentos que precisam ser estimados com precisão para que os componentes e operação do poço tenham um bom desempenho. Quando há um $kick$ (influxo para dentro do poço de fluidos confinados nas rochas porosas), cada elemento do CSB precisa funcionar de forma independente com o objetivo de evitar um $blowout$ (influxo descontrolado), causando danos aos equipamentos da sonda, assim como lesões às pessoas que trabalham nela. Os revestimentos que compõe um poço de petróleo são parte do CSB. Nesse contexto, este trabalho propõe um estudo de caso para avaliar as resistências dos revestimentos sujeitos a perda total de circulação ou $kick$ de gás por meio de análises triaxiais, utilizando o Sistema de Aplicações de Engenharia de Petróleo (SAEP). No caso de estudo proposto neste trabalho, para o cenário de perda total de circulação, o revestimento condutor atendeu as envoltórias de von Mises e API; o revestimento de superfície sofreu plastificação e não atendeu aos coeficientes de segurança. Para um cenário de $kick$ de gás, o revestimento intermediário ficou fora do domínio seguro. De acordo com os resultados alcançados, são sugeridos outros revestimentos para adequar as resistências dentro do domínio seguro das envoltórias de von Mises e API.

\vspace{18pt} % <- keep this vertical space!

\noindent \textbf{Palavras-chave:} Conjunto Solidário de Barreiras, Perda Total de Circulação, $Kick$ de gás
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.35]{Figures/CSB_corrigido.png}
\vspace{12pt}
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.45]{Figures/elipse.png}
%\vspace{12pt}
...
segurança API \citep{santos2021desenvolvimento}. Os procedimentos apresentados anteriormente são a base dos cálculos realizados pelo CWELL a fim de avaliar ou dimensionar revestimentos com a possibilidade de adicionar os fatores de segurança. A ferramenta também gera os esforços. Não é o foco deste trabalho detalhar suas formulações matemáticas, porém em \citep{costa} são descritas.
...
A perda de circulação é a invasão de fluido de perfuração para a formação através de fraturas existentes ou provocadas em formações com alta permo-porosidade (devido à presença de formação inconsolidada, existência de falhas, fraturas naturais, cavernas, entre outros) ou em zonas depletadas \citep{Chieza2011}. Na perda de circulação total não há retorno do fluido para a superfície. Neste caso, tem-se uma falha no CSB primário (fluido de perfuração), pois a invasão da lama na formação reduz o nível de fluido de perfuração no anular, diminuindo a pressão hidrostática \citep{Chieza2011}. Caso pressão hidrostática alcançar níveis abaixo da pressão de poros, a perda total de circulação pode ocasionar um \textit{kick}.

Durante o \textit{kick} pode ocorrer a invasão de óleo, água ou gás provenientes da formação perfurada. A situação mais crítica ocorre quando a invasão é feita por gás. Esse fato se deve a sua maior expansão quando este está se deslocando em direção a superfície, causando uma maior diminuição da densidade da mistura \citep{da2011tecnologia}.

Admitindo que a barreira primária seja rompida, as resistências dos revestimentos que compõem o poço são analisadas, observe na Figura~\ref{Fig2} que o revestimento é um dos elementos dos CSB secundário.
...
Superfície & 13 3/8 & 0 & 0 & 1400 & 17 1/2 & 17.5 & K55 & 61 \\
...
A Figura~\ref{fig:kickgas} detalha os dados utilizados para a simulação de um \textit{kick} de gás na interface SAEP. Os valores numéricos dos parâmetros foram extraídos de \citep{costa2}. Não é necessário atribuir valores numéricos para simular a perda total de circulação no CWELL.
...
A Tabela \ref{tab_res} apresenta as resistências dos revestimentos de poço obtidas pelo CWELL. Do revestimento condutor ao intermediário nota-se uma diminuição da resistência à tração. Por outro lado, as resistências por $burst$ e colapso aumentam na direção do revestimento condutor ao intermediário. Assim, há uma tendência de o dimensionamento ser focado em esforços mais acentuados por tração no revestimento condutor e em esforços mais acentuados por pressão nos revestimentos de superfície e intermediário. 
...
Na Figura \ref{fig:graph5} é apresentada a análise triaxial do revestimento intermediário (Tubo C75), considerando o cenário de $kick$ de gás. Assim como no revestimento de superfície, parte da curva relativa ao carregamento fica fora do domínio de interseção das envoltórias de segurança, significando deformações plásticas na coluna e o não atendimento a resistência por $burst$, a qual é minorada pelos fatores de segurança. Novamente propõe-se um novo grau para o tubo, ou seja, considerando um projeto conservador quanto a estas deformações. Assim, foram avaliados os tubos com graus L80, N80, C90 e C95. No último tubo os esforços por $kick$ ficaram dentro das envoltórias de segurança, conforme ilustrado na Figura \ref{fig:resultsF4}(a). Também sugere-se o uso de seções combinadas.


\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.2]{Figures/fase3/tri.png}
\vspace{12pt}
\caption{\centering{Análise triaxial de revestimento intermediário.}}
\label{fig:graph5}
\end{center}
\end{figure}
\vspace{8pt}
...
Chama-se a atenção para a análise triaxial do tubo C90 mostrado na Figura \ref{fig:resultsF4}(b). Todos os esforços estão no interior da elipse de von Mises. Em outra direção, o ponto com esforço axial mínimo encontra-se sobre a curva limite API. Isso significa que não há plastificação do revestimento, porém os coeficientes de segurança estão no limite de serem violados, indicando um risco dada a possibilidade de incertezas nos processos de fabricação que podem reduzir a própria resistência indicada pelo fabricante.

\begin{figure}[H]
\begin{center}
\center
\subfigure[ref1][]{\includegraphics[width=7.6cm]{./Figures/fase3/c95.png}}\qquad
\subfigure[ref2][]{\includegraphics[width=7.6cm]{./Figures/fase3/c90.png}}
%\subfigure[ref3][]{\includegraphics[width=7.6cm]{./Figures/fase3/tracao.png}}
\caption{\centering{Análise triaxial de revestimento intermediário: (a) C95 e (b) C90.}}
\label{fig:resultsF4}
\end{center}
\end{figure}
...
%\begin{figure}[H]
%\begin{center}
%\includegraphics[scale=0.215]{Figures/fase3/c90.png}
%\vspace{12pt}
%\caption{\centering{Análise triaxial de revestimento %intermediário com grau C90.}}
%\label{fig:graph5}
%\end{center}
%\end{figure}
%\vspace{8pt}
...
%Além disso, diferentemente dos cenários de perda de circulação total onde eram predominantes esforços de tração observa-se que o cenário de $kick$ de gás causa tração e compressão no revestimento, conforme a Figura \ref{fig:graph5}. Na Figura \ref{fig:graph2} é detalhada a força de tração ao longo da profundidade.  Desta forma, entre as profundidades de 2500 a 3400 m há compressão. Sendo assim, uma investigação de flambagem nessa região da coluna é válida. Ademais, note que, com base na Tabela \ref{tab_res}, a resistência à tração do tubo C75 não é violada ao longo da região tracionada.

%\begin{figure}[H]
%\begin{center}
%\includegraphics[scale=0.215]{Figures/fase3/tracao.png}
%\vspace{12pt}
%\caption{\centering{Esforços de tração no revestimento intermediário.}}
%\label{fig:graph2}
%\end{center}
%\end{figure}
%\vspace{8pt}
...
Este trabalho realizou um estudo de caso sobre a integridade do elemento de barreira de poço revestimento durante cenários de perfuração. Os cenários avaliados foram perda de circulação total e $kick$ de gás. O revestimento condutor, considerando o primeiro cenário, atendeu as envoltórias de von Mises e API. Por outro lado, nessa mesma operação o revestimento de superfície sofreu plastificação e não atendeu aos coeficientes de segurança. Analogamente, o revestimento intermediário sob um cenário de $kick$ também ficou fora do domínio seguro. Assim, foram avaliados diferentes graus para os tubos dos revestimentos de superfície e intermediário, considerando projetos mais conservadores, ou seja, que não aceitam a plastificação na coluna e a violação dos fatores de segurança. Deste modo, indicou-se tubos com grau C90 e C95, respectivamente. Buscando economia e segurança, ainda sugeriu-se a utilização de seções compostas em ambos os revestimentos. Destacam-se ainda as funcionalidades do CWELL fornecendo análises triaxiais, resistências e esforços, os quais são úteis para dimensionamento de colunas de revestimento, bem como verificação da integridade. 
