...
% --------------------------------------------------------------------------
% PLEASE, EDIT THIS!
\fancyhead[LE]{\footnotesize \textit{Avaliação da integridade de barreiras de poço primária e secundária em cenários de $kick$ de gás e perda de circulação total}}
...
\noindent
\textbf{\Large
Avaliação da integridade de barreiras de poço primária e secundária em cenários de \textit{kick} de gás e perda de circulação total} 
...
\noindent onde $A$, $B$, $C$, $F$ e $G$ são constantes obtidas por equações empíricas, conforme a seguir:
...
É importante destacar que o regime de colapso vigente no revestimento é determinado conforme a relação entre o diâmetro externo e a espessura do revestimento, além de equações empíricas envolvendo as constantes $A$, $B$, $C$, $F$ e $G$. Em API TR 5C3 \citep{american2008technical} são apresentadas as relações para cada regime. Também é possível avaliar a resistência de forma tridimensional, adaptando o critério de falha de von Mises para os cenários específicos de $burst$ e colapso. Em \citep{costa}, por exemplo, é mostrado o passo a passo. Além disso, a API TR 5C3 \citep{american2008technical} fornece uma envoltória de segurança, a qual é resultado das resistências acima e de fatores de segurança. Esses dois métodos formam uma análise triaxial.
...
segurança API \citep{santos2021desenvolvimento}. Na Figura \ref{Eq1} é apresentada a elipse de resistência triaxial para tubos de revestimento API.


\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.5]{Figures/elipse.png}
%\vspace{12pt}
\caption{Elipse de resistência triaxial. Fonte: \cite{costa}.}
\label{poco}
\end{center}
\end{figure}
...
O cenário de estudo adotado é apresentado na Figura~\ref{fig:poco}. A Tabela~\ref{tab:poco} apresenta os valores numéricos associados a configuração do poço adotado. Este caso de estudo foi idealizado por \citep{costa}. Neste trabalho é avaliada a perda total de circulação nas fases 1 (revestimento condutor) e 2 (revestimento intermediário). Adicionalmente, na terceira fase (revestimento de produção), é avaliado um cenário de \textit{kick} de gás.  
...
%\vspace{12pt}
\caption{Dados para simulação de um \textit{kick} de gás na interface SAEP.}
\label{fig:kickgas}
\end{center}
\end{figure}
